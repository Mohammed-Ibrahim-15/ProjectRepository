...
function createPrescription($d_id, $p_id)
{
    $conn = db_conn();
    $query = "INSERT into prescription (d_id, p_id) 
           VALUES (:d_id, :p_id)";
    try {
        $stmt = $conn->prepare($query);
        $stmt->execute([
...
function returnPrescription($p_id, $d_id)
{
    $conn = db_conn();
    $selectQuery = "SELECT pr_id FROM `prescription` WHERE p_id = '$p_id' AND d_id = '$d_id'";
...
{
    $conn = db_conn();
    $selectQuery = "SELECT m_name FROM `medicine`";
